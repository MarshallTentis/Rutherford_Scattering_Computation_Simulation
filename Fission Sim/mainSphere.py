...
def getInduced(m,s):
    ...
    #----Check radius from center, if >R, the no induce fission----
    r_final1 = np.sqrt((xFinal_1)**2+(yFinal_1)**2+(zFinal_1)**2)
    r_final2 = np.sqrt((xFinal_2)**2+(yFinal_2)**2+(zFinal_2)**2)
    if r_final1 <= R:
        N_i += 1
    if r_final2 <= R:
        N_i += 1
...
#---file stuff---
output_dir = "sphereResults"
output_file = f"{output_dir}/critical_sphere_mass_results.txt"
